...
void searchMedia(vector<Media*> & MediaDataBase){ // search Media 
  char searchInput[100]; //initializing variables used for this method 
    int searchNumInput; 
    int searchType; 
	...
	if (strcmp(searchInput, MediaDataBase[i]->gettitle()) == 0) { //if input matches the title name, get title
	  MediaDataBase[i]->print(); //print it out
}
      }
      ...
      cin>>searchNumInput; 
      for (int i = 0; i < MediaDataBase.size(); i++) {
        if (MediaDataBase[i]->getInt() == searchNumInput) { // if input matches the year, print media that has that year 
